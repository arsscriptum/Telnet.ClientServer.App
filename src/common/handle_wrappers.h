	...
	namespace INTERNAL{
		...
		inline HWND addclipboardlistener(HWND h){
			if (!AddClipboardFormatListener(h))
				LOG_ERROR("SOCKETWrapper","AddClipboardFormatListener Failed");
			return h;
		}
	}
	struct SOCKETWrapper{
...
#define RAIIHFONT(handle) std::unique_ptr<std::remove_pointer<HFONT>::type, decltype(&::DeleteObject)>(handle, &::DeleteObject)	
	...
	typedef std::unique_ptr<std::remove_pointer<HWND>::type, decltype(&INTERNAL::dialogboxcleanup)> RAIIDIALOGBOX_TYPE;
#define RAIIDIALOGBOX(handle) std::unique_ptr<std::remove_pointer<HWND>::type, decltype(&RemoteDesktop::INTERNAL::dialogboxcleanup)>(handle, &RemoteDesktop::INTERNAL::dialogboxcleanup)	
	...
	typedef std::unique_ptr<HWNDTimer, decltype(&INTERNAL::hwndtimercleanup)> RAIIHWNDTIMER_TYPE;
